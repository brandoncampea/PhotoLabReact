...
SETTINGS=(
  ...
  "APP_BASE_URL=${APP_BASE_URL:-$APP_URL}"
)
...
if [[ -n "${SMTP_HOST:-}" ]]; then
  ...
  SETTINGS+=("SMTP_PORT=${SMTP_PORT:-587}")
  SETTINGS+=("SMTP_USER=${SMTP_USER:-}")
  SETTINGS+=("SMTP_PASSWORD=${SMTP_PASSWORD:-}")
  SETTINGS+=("SMTP_SECURE=${SMTP_SECURE:-false}")
  SETTINGS+=("SMTP_FROM=${SMTP_FROM:-Photo Lab <no-reply@yourdomain.com>}")

  if [[ -n "${SMTP_REPLY_TO:-}" ]]; then
    SETTINGS+=("SMTP_REPLY_TO=${SMTP_REPLY_TO}")
  fi
fi
