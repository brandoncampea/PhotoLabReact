    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        {rows.map((row) => {
          ...
          return (
            <div key={row.orderId} style={{ border: '1px solid #e2e8f0', borderRadius: 12, background: '#fff', overflow: 'hidden' }}>
              <div style={{ padding: 16, borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontSize: 20, fontWeight: 700 }}>
                    <Link to={`/admin/orders?orderId=${row.orderId}`}>Order #{row.orderId}</Link>
                  </div>
                ...
                <div style={{ minWidth: 260, display: 'grid', gap: 4, fontSize: 14 }}>
                  <div><strong>Expected:</strong> {formatCurrency(row.audit?.summary?.expectedTotalCost ?? 0)}</div>
                  <div><strong>WHCC returned:</strong> {formatCurrency(row.audit?.summary?.actualTotalCost ?? 0)}</div>
                  <div><strong>Difference:</strong> {formatCurrency(row.audit?.summary?.differenceAmount ?? 0)}</div>
                  <div><strong>Mismatches:</strong> {row.audit?.summary?.mismatchCount ?? 0}</div>
                </div>
              </div>
              <div style={{ padding: 16, display: 'grid', gap: 10 }}>
                {mismatches.map((item, index) => (
                  <div key={`${row.orderId}-${item.localItemId || index}`} style={{ padding: 12, borderRadius: 10, border: '1px solid #fed7aa', background: '#fff7ed' }}>
                    ...
                    <div style={{ marginTop: 8, display: 'grid', gap: 4, fontSize: 14 }}>
                      <div><strong>Expected line cost:</strong> {formatCurrency(item.expectedLineCost)}</div>
                      <div><strong>Returned line cost:</strong> {formatCurrency(item.actualLineCost)}</div>
