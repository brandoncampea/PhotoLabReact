...
const PLAYWRIGHT_TEST_USER_PASSWORD = 'playwrightpass';
// Ensure Playwright test user exists
const ensurePlaywrightTestUser = async () => {
  const hash = await bcrypt.hash(PLAYWRIGHT_TEST_USER_PASSWORD, 10);
  let user = await queryRow('SELECT TOP 1 id FROM users WHERE email = $1', [PLAYWRIGHT_TEST_USER_EMAIL]);
  if (!user) {
    user = await queryRow(
      `INSERT INTO users (email, password, name, role, is_active, created_at)
       VALUES ($1, $2, $3, $4, 1, CURRENT_TIMESTAMP)
       RETURNING id`,
      [PLAYWRIGHT_TEST_USER_EMAIL, hash, 'Playwright Test User', 'customer']
    );
    console.log('Playwright test user created.');
  } else {
    await query(
      `UPDATE users SET password = $1, name = $2, role = $3, is_active = 1 WHERE id = $4`,
      [hash, 'Playwright Test User', 'customer', user.id]
    );
    console.log('Playwright test user updated.');
  }
...
const ensureSeedProductAndSize = async (priceListId) => {
  ...
  return {
    ...
    productName: 'Playwright Test Product',
    sizeName: '8x10',
  };
};
