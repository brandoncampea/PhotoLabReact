  ...
  try {
    ...
    const items = await queryRows(
      `SELECT
         ...
         sii.product_size_id as productSizeId,
         sii.quantity,
       ...
       LEFT JOIN product_sizes ps ON ps.id = sii.product_size_id
...
router.get('/admin/all', superAdminRequired, async (req, res) => {
  try {
    const studioBalances = await queryRows(
      `SELECT
         s.id as studioId,
    ...
    res.json({ studioBalances, totalOutstanding });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
